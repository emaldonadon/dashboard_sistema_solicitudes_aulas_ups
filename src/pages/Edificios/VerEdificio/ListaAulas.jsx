...
import { DeleteAula } from "./DeleteAula";
import { InactivarAula } from "./InactivarAula";
...
export const ListaAulas = () => {
    ...
    const LoadData = (startAfterDoc, persistMessages = []) => {
        ...
        /* let ref = db.collection("edificios").doc(params.edificio).collection('aulas').where('estado', '==', 0); */

        let ref = db.collection("sedes").doc(params.sede).collection('campus').doc(params.campus).collection('edificios').doc(params.edificio).collection('aulas').where('estado', '==', 0);
    ...
    const columns = [
        { field: 'piso', headerName: 'PISO ', width: 100 },
        { field: 'aula', headerName: 'AULA', width: 165 },
        {
        ...
        {
            field: '1', headerName: 'INHABILITAR', width: 120,
            renderCell: (params) => {
                return <InactivarAula props={params.row} />
            }
        },
        {
            field: '', headerName: 'ELIMINAR', width: 120,
            renderCell: (params) => {
                return <DeleteAula props={params.row} />
            }
        },
    ];
