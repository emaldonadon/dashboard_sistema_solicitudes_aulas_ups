...
export const SolicitudesAulasEspera = () => {
    ...
    const LoadData = (startAfterDoc, persistMessages = []) => {
        ...
        if (auth.currentUser.rol === 'ADMINISTRADOR') {
            ...
            if (endDate && startDate) {
                let ref = db.collection("solicitudes_aulas").where('estado', '==', 0);
                ...
                ref = ref.where('dia', '>=', startfulldate)
                ref = ref.where('dia', '<=', endfulldate)
            ...
            if (endDate && startDate) {
                let ref = db.collection("solicitudes_aulas").where('uid_usuario', '==', auth.currentUser.uid).where('estado', '==', 0);
                startDate.setHours(0, 0, 0, 0)
                endDate.setHours(23, 59, 0, 0)
                var startfulldate = firebase.firestore.Timestamp.fromDate(startDate);
                var endfulldate = firebase.firestore.Timestamp.fromDate(endDate);
    ...
    const columns = [
        ...
        { field: 'aula', headerName: 'AULA', width: 150 },
        {
            ...
            renderCell: (params) => {
                const respuesta = params.row.tipo_ticket === 'solicitud_aula' ? 'SOLICITUD AULA' : 'SOLICITUD LABORATORIO';
                return `${respuesta}`
            }
        },
        { field: 'informacion_adicional', headerName: 'INFORMACION ADICIONAL', width: 300 },
        ...
        {
            field: 'dia', headerName: 'DIA SOLICITADO', type: 'dateTime', width: 170,
    ...
    const handleExport = () => {
        ...
        const reData = [
            ...data.map(doc => {
                return {
                    ...
                    BLOQUE: doc.bloque,
                    AULA: doc.aula,
                    TIPO_SOLICITUD: doc.tipo_ticket === 'solicitud_aula' ? 'SOLICITUD AULA' : 'SOLICITUD LABORATORIO',
                    INFORMACION_ADICIONAL: doc.informacion_adicional,
                    USUARIO_QUE_ABRIO_SOLICITUD: doc.usuario_que_abrio_ticket,
                    CORREO_DE_USUARIO_SOLICITANTE: doc.email_usuario,
                    DIA_SOLICITADO: doc.dia.toDate(),
        ...
        XLSX.writeFile(wb, `Solicitudes_Aulas_En_Espera ${date1}.xlsx`)
    }
        ...
        <>
            <Box pl={1}>
                <p style={{ marginBottom: "0px", marginTop: "0px", fontSize: "14px" }}>
                    SOLICITUDES EN ESPERA (<strong>{data && data.length}</strong>)
                </p>
